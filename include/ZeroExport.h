...
public:
    ...
    // Return the configured per-inverter minimum multiplied by the inverter count.
...
private:
    ...
    struct InverterState {
        ...
        // A new/unknown inverter starts at the safe 50 W floor. This is
        ...
        // sending the 50 W initialization command again.
        bool StartupAtFullPower = false;
        // Set when a grid-source outage starts and cleared only after the
        // 50 W fail-safe command is sent to this inverter. This is per
        // inverter because reachability can change independently.
    ...
    void sendKeepAlive();
    // When the grid source is unavailable, cap reachable inverters at the
    // minimum commanded limit to reduce the risk of export. Unreachable
    // inverters are retried when they return; the commands are rate-limited.
    void applyGridUnavailableFailSafe();
    ...
    uint32_t _lastFailSafe = 0; // millis() timestamp of the last source-loss fail-safe resend.
    bool _gridFailSafeActive = false; // Source has no value; limits are held at the minimum.
