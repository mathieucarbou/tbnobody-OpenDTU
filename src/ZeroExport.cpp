...
#define ZEROEXPORT_MIN_INVERTER_POWER 50.0f // W per inverter (auto mode)
...
void ZeroExportClass::applyGridUnavailableFailSafe()
{
    ...
    for (uint8_t i = 0; i < numInverters; i++) {
        ...
        const float safeLimit = std::min<float>(ZEROEXPORT_MIN_INVERTER_POWER, maxPower);
...
void ZeroExportClass::regulate()
{
    ...
    float correction = missedPower;
    const uint32_t sourcePeriodMs = source->getUpdatePeriodMs();
    if (missedPower > 0 && sourcePeriodMs > 0) {
    ...
    float newTotalLimit = _totalLimit + correction;
