...
void WebApiZeroExportClass::onZeroExportAdminPost(AsyncWebServerRequest* request)
{
    ...
    if (root["shelly_lnm_type"].as<String>().length() == 0
        || root["shelly_lnm_type"].as<String>().length() > ZEROEXPORT_MAX_SHELLY_LNM_TYPE_STRLEN) {
        retMsg["message"] = "Shelly LNM type must between 1 and " STR_EXTRACT(ZEROEXPORT_MAX_SHELLY_LNM_TYPE_STRLEN) " characters long!";
        retMsg["code"] = WebApiError::ZeroExportTypeLength;
        retMsg["param"]["max"] = ZEROEXPORT_MAX_SHELLY_LNM_TYPE_STRLEN;
        WebApi.sendJsonResponse(request, response, __FUNCTION__, __LINE__);
        return;
    }
