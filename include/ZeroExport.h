...
private:
    ...
    // this is the only place applyConfig() is called from, and the only
    // place the configuration is mutated without the write guard (which is
    // safe from the main-loop task; see mutableConfig() in ZeroExport.cpp).
    ...
    // regulate() sends and retries the commands from the inverter state
    // machine while Zero-Export is disabled.
    void releaseLimits();
    ...
    std::atomic<bool> _mqttEnabledPending { false };
    std::atomic<bool> _mqttEnabledValue { false };
    std::atomic<bool> _mqttSetPointPending { false };
    std::atomic<int32_t> _mqttSetPointValue { 0 };
};
